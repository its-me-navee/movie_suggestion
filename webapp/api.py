...
def get_movie(movie_id):
    ...
    movie_query = '''SELECT * FROM movies WHERE id = %s;'''
    ...
    try:
        ...
        cursor.execute(language_query,(movie_id,))
        languages = []
        for row in cursor:
            languages.append(row)
        movie_dict['languages'] = languages

        cursor.execute(countries_query,(movie_id,))
        countries = []
        for row in cursor:
            countries.append(row)
        movie_dict['countries'] = countries

        cursor.execute(companies_query,(movie_id,))
        companies = []
        for row in cursor:
